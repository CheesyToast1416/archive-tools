...
def _set_combo_codec(combo: EncodingComboBox, codec: str) -> None:
    """Select the item whose data matches *codec* (empty string → index 0)."""
    for i in range(combo.count()):
        if (combo.itemData(i) or "") == codec:
...
class SettingsDialog(QDialog):
    ...
    def _load_from_settings(self) -> None:
        ...
        _set_combo_codec(self._pwd_enc, s.default_password_encoding)
        _set_combo_codec(self._fname_enc, s.default_filename_encoding)
